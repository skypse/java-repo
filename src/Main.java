//import operadores_relacionais_java_desafio_pratico11_alura.OperadoresRelacionais;
import perfiljavadesafiopratico11_alura.Perfil;
import concatenacao_strings_java_pratica_alura.ConcatenacaoExemplos;
/* import estudos_java_desafio_pratico11_alura.Estudos;
import operacoes_java_desafio_pratico11_alura.Operacoes;
import operadores_logicos_java_desafio_pratico11_alura.OperadoresLogicos;
import operadores_de_incremento_java_desafio_pratico11_alura.OperadoresIncremento;
import movies_description_java_pratica_alura.Movies;*/

public class Main {
    public static void main(String[] args) {
        // classe perfil
        Perfil saudacao = new Perfil();
        saudacao.saudacaoPerfil();

        /* classe estudos
        //Estudos estudos = new Estudos();
        //estudos.anotacoesEstudos();
        //System.out.println("------------------------------------------------------------");

        // classe operacoes
        //System.out.println("getting Class named 'operacoes' \n Example how the code are counting 10 (operator) 5");
        //System.out.println("Soma: " + Operacoes.somar(10, 5));         // Soma: 10
        //System.out.println("Subtração: " + Operacoes.subtrair(10, 5)); // Subtração: 4
        //System.out.println("Multiplicao: " + Operacoes.multiplicar(10, 5)); // Multiplicacao: 50
        //System.out.println("Dividir: " + Operacoes.dividir(10, 5)); // Dividir: 2
        //System.out.println("Modulo (a = 10, b= 3): " + Operacoes.modulo(10, 3)); // Resto da divisao: 1

        // operadores relacionais
        //OperadoresRelacionais.exibirTextoInformativo();

        // operadores logicos
        //OperadoresLogicos.operadorAnd();
        //OperadoresLogicos.operadorNot();
        //OperadoresLogicos.operadorOr();

        // operadores de incremento
        OperadoresIncremento.preIncremento();
        OperadoresIncremento.posIncremento();

        // movie desc:
        Movies.movieDescription();*/

        // concatenção strings
        ConcatenacaoExemplos.testandoString();
        ConcatenacaoExemplos.comparandoString();
    }
}